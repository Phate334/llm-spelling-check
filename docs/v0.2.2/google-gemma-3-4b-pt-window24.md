...
calls: 263
input_chars: 54462
output_chars: 1195675
```

依用途累計：

```text
{"score_original": 24, "fim_candidates": 59, "score_window": 180}
```

詳細 vLLM 每次呼叫紀錄：`google-gemma-3-4b-pt-window24-calls.jsonl`

## 案例結果

```text
case  status     eval      calls top_candidate
01    uncertain  uncertain 27    圜->館 delta=0.608 source=vllm_top_logprob
02    no_error   missed    20    -
03    uncertain  uncertain 24    提->屜 delta=0.857 source=vllm_top_logprob
...
07    uncertain  uncertain 27    汽->氣 delta=1.796 source=vllm_top_logprob
08    uncertain  uncertain 26    李->編 delta=0.600 source=vllm_top_logprob
09    uncertain  uncertain 22    結->節 delta=0.579 source=vllm_top_logprob
10    no_error   ok        18    -
11    no_error   ok        14    -
12    no_error   ok        16    -
```
...
- 每次呼叫的用途、主要 payload 欄位、回傳重點欄位已寫入 `google-gemma-3-4b-pt-window24-calls.jsonl`。
