...
def spelling_check(text: str, client: PromptScorer, config: SpellingCheckConfig) -> CorrectionResult:
    ...
    suspicious = select_suspicious_chars(risks, threshold=config.risk_threshold, limit=config.suspicious_limit)

    corrections: list[CandidateCorrection] = []
    window_score_cache: dict[str, float] = {}
    for risk in suspicious:
        candidates = generate_candidates(text, risk, original_tokens, limit=config.candidate_limit)
        original_window = local_window(text, risk.index, config.window_radius)
        original_score = _score_window(original_window, client, config, window_score_cache)

        for candidate in candidates:
            candidate_text = replace_char(text, candidate.index, candidate.candidate_char)
            candidate_window = local_window(candidate_text, candidate.index, config.window_radius)
            candidate_score = _score_window(candidate_window, client, config, window_score_cache)
            corrections.append(
                CandidateCorrection(
...
def _score_window(window: str, client: PromptScorer, config: SpellingCheckConfig, cache: dict[str, float]) -> float:
